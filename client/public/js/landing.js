...
function typeToBadge(types) {
    const t = types[0] ?? '';
    if (t === 'Performance Hall')
        return { abbr: 'HALL', cls: 'badge-teal' };
    if (t === 'Gallery')
        return { abbr: 'GALL', cls: 'badge-purple' };
    if (t === 'Studio')
        return { abbr: 'STUD', cls: 'badge-orange' };
    if (t === 'Dance')
        return { abbr: 'DANC', cls: 'badge-pink' };
    if (t === 'Rehearsal')
        return { abbr: 'REHS', cls: 'badge-teal' };
    if (t === 'Workshop')
        return { abbr: 'WORK', cls: 'badge-green' };
    ...
    try {
        ...
        if (spacesRes.ok) {
            ...
            if (data.spaces?.length) {
                ...
                if (statSpacesEl)
                    statSpacesEl.textContent = data.spaces.length.toLocaleString();
            }
        }
        if (statsRes.ok) {
            ...
            if (dbSpaces)
                dbSpaces.textContent = (stats.totalSpaces ?? 0).toLocaleString();
            const dbConfirmed = document.getElementById('dbStatConfirmed');
            if (dbConfirmed)
                dbConfirmed.textContent = (stats.bookingsConfirmed ?? 0).toLocaleString();
            const dbRequested = document.getElementById('dbStatRequested');
            if (dbRequested)
                dbRequested.textContent = (stats.bookingsRequested ?? 0).toLocaleString();
            const dbCities = document.getElementById('dbStatCities');
            if (dbCities)
                dbCities.textContent = (stats.totalCities ?? 0).toLocaleString();
